...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Crypto YouTube News Summarizer")
...
def find_crypto_channels(youtube_api: YouTubeAPI, search_queries: List[str], max_channels: int) -> List[Dict[str, Any]]:
    """
    ...
    """
    logger.info(f"Searching for crypto YouTube channels using {len(search_queries)} queries")
    ...
    for query in search_queries:
        ...
        for channel in channels:
            ...
            if is_relevant:
                ...
                logger.info(f"Found relevant crypto channel: {channel['title']} (ID: {channel_id})")
...
def download_channel_videos(
    ...
    logger.info(f"Getting recent videos for channel: {channel_title} (ID: {channel_id})")
    
    videos = youtube_api.get_channel_videos(channel_id, max_results=videos_per_channel)
    ...
    logger.info(f"Downloaded {len(downloaded_videos)} videos for channel: {channel_title}")
...
def process_videos(
    ...
    for video in videos:
        ...
        if transcript:
            transcript_path = transcript_extractor.save_transcript(video, transcript, transcript_dir)
            
            if transcript_path:
                video['transcript_path'] = transcript_path
                
                summary_info = content_summarizer.summarize_transcript(video, transcript)
                
                if summary_info:
                    summary_path = content_summarizer.save_summary(summary_info, summary_dir)
                    ...
                    if summary_path:
                        ...
                        logger.warning(f"Failed to save summary for video: {video_title}")
                else:
                    logger.warning(f"Failed to generate summary for video: {video_title}")
            else:
                logger.warning(f"Failed to save transcript for video: {video_title}")
        else:
            logger.warning(f"Failed to extract transcript for video: {video_title}")
...
def run_workflow(args):
    ...
    try:
        ...
        channels = find_crypto_channels(youtube_api, search_queries, args.max_channels)
        ...
        for channel in channels:
            ...
            if args.skip_download:
                logger.info(f"Skipping video download for channel: {channel_title}")
                videos = []
            else:
            ...
            if not videos:
                logger.warning(f"No videos to process for channel: {channel_title}")
                continue
            
            summaries = process_videos(transcript_extractor, content_summarizer, videos)
            
            if summaries:
                all_summaries.extend(summaries)
                logger.info(f"Added {len(summaries)} summaries for channel: {channel_title}")
            else:
                logger.warning(f"No summaries generated for channel: {channel_title}")
...
def main():
    ...
    try:
        ...
        logger.error(f"Error running Crypto YouTube News Summarizer: {e}", exc_info=True)
