...
class ContentSummarizer:
    ...
    def summarize(self, text: str, min_length: int = None, max_length: int = None) -> str:
        """
    ...
    def summarize_transcript(self, video_info: Dict[str, Any], transcript: str) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        if not transcript:
            logger.warning(f"No transcript provided for video: {video_info['id']}")
            return None
        ...
        try:
            ...
            logger.info(f"Successfully summarized transcript for video: {video_info['id']}")
            return summary_info
        
        except Exception as e:
            logger.error(f"Error summarizing transcript for video {video_info['id']}: {e}")
            return None
    
    def save_summary(self, summary_info: Dict[str, Any], output_dir: str) -> Optional[str]:
        """
        ...
        channel_dir = os.path.join(output_dir, self._sanitize_filename(channel_title))
        if not os.path.exists(channel_dir):
            os.makedirs(channel_dir)
        
        summary_filename = f"{self._sanitize_filename(video_title)}_{video_id}_summary.txt"
