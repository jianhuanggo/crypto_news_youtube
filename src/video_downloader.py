...
class VideoDownloader:
    ...
    def download_video(self, video_info: Dict[str, Any]) -> Optional[str]:
        ...
        channel_dir = os.path.join(self.download_dir, self._sanitize_filename(channel_title))
        ...
        try:
            ...
            if not stream:
                logger.warning(f"No suitable stream found for video: {video_id}")
                return None
            
            logger.info(f"Downloading video: {video_info['title']} ({video_id})")
